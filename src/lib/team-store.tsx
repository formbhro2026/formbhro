...
import { subscribeToRoom, subscribeToRequests, subscribeToMyNotifications, sendTyping } from "@/lib/api/realtime";
...
type TeamStore = {
  ...
  attachDocument: (
    requestId: string,
    file: { name: string; size: string; kind: TeamDocument["kind"]; previewUrl?: string; blob?: File }
  ) => void;
...
export function TeamStoreProvider({ children }: { children: ReactNode }) {
  ...
  const startTyping = useCallback(
    (requestId: string, duration: number) => {
  ...
  const setDelivery = useCallback((messageId: string, delivery: TeamDelivery) => {
    setMessages((prev) => prev.map((m) => (m.id === messageId ? { ...m, delivery, read: delivery === "read" } : m)));
  }, []);
  ...
  const runDeliveryCycle = useCallback(
    (requestId: string, messageId: string, attempt = 1) => {
      setMessages((prev) =>
        prev.map((m) =>
          m.id === messageId
            ? { ...m, delivery: attempt === 1 ? "sending" : "retrying", attempts: attempt, deliveryError: undefined }
            : m
        )
      );
  ...
  const failedFor = useCallback(
    (requestId: string) => messages.filter((m) => m.requestId === requestId && m.delivery === "failed").length,
    [messages]
  );
    ...
    (async () => {
      ...
      try {
        ...
        if (raw) {
          ...
          stored = "member" in parsed ? (parsed as { member: TeamMember; live?: boolean }) : { member: parsed as TeamMember };
        }
      } catch {
        /* ignore */
      }
  ...
  const signIn = useCallback(
    async (email: string, password: string, remember: boolean) => {
      try {
        ...
        return { ok: false, error: err instanceof Error ? err.message : "Invalid email or password. Contact your administrator if you need help." };
      }
    },
    [persistSession]
  );
  ...
  const signInWithCode = useCallback(
    async (code: string, remember: boolean) => {
      try {
        ...
        const { data: authData, error: authError } = await signInWithPassword(res.email, "FBH-Team@2026");
        ...
        return { ok: false, error: err instanceof Error ? err.message : "Invalid team code. Please check your credentials." };
      }
    },
    [persistSession]
  );
  ...
  const updateMember = useCallback((patch: Partial<TeamMember>) => {
    setMember((m) => {
      ...
      try {
        ...
        if (target.getItem(SESSION_KEY)) target.setItem(SESSION_KEY, JSON.stringify({ member: next, live: liveRef.current }));
      } catch {
        /* ignore */
      }
      return next;
  ...
  const assigned = useMemo(
    () =>
      member
        ? requests.filter((r) => r.assigneeId === member.id).map((r) => ({ ...r, unread: unreadByRequest[r.id] ?? 0 }))
        : [],
  ...
  const visibleDocuments = useMemo(() => documents.filter((d) => assignedIds.has(d.requestId)), [documents, assignedIds]);
  const visibleMessages = useMemo(() => messages.filter((m) => assignedIds.has(m.requestId)), [messages, assignedIds]);

  const getRequest = useCallback((id: string) => assigned.find((r) => r.id === id), [assigned]);
  const messagesFor = useCallback((id: string) => visibleMessages.filter((m) => m.requestId === id), [visibleMessages]);
  const documentsFor = useCallback((id: string) => visibleDocuments.filter((d) => d.requestId === id), [visibleDocuments]);
  const getDocument = useCallback((id: string) => visibleDocuments.find((d) => d.id === id), [visibleDocuments]);

  const touch = useCallback((requestId: string, patch: Partial<TeamRequest>, timelineLabel?: string) => {
    setRequests((prev) =>
      prev.map((r) =>
        r.id === requestId
          ? {
              ...
              timeline: timelineLabel ? [...r.timeline, { label: timelineLabel, time: `Today • ${nowTime()}` }] : r.timeline,
            }
          : r
      )
    );
  ...
  const sendMessage = useCallback(
    (requestId: string, text: string) => {
      ...
      setMessages((prev) => [
        ...prev,
        { id, requestId, author: "team", authorName: name, time: nowTime(), text, read: false, delivery: "sending" },
      ]);
      ...
      if (liveRef.current) {
        ...
        if (!room?.chatRoomId) {
          setMessages((prev) =>
            prev.map((m) => (m.id === id ? { ...m, delivery: "failed", deliveryError: "Chat room unavailable." } : m))
          );
          return;
        }
        void messagesApi
          ...
          .then((row) => {
            setMessages((prev) => {
              ...
              return prev.map((m) => (m.id === id ? { ...m, id: row.id, delivery: "delivered" } : m));
            });
          })
          .catch(() => {
            setMessages((prev) =>
              prev.map((m) =>
                m.id === id ? { ...m, delivery: "failed", deliveryError: "Message not delivered." } : m
              )
            );
          });
        return;
  ...
  const attachDocument = useCallback(
    (requestId: string, file: { name: string; size: string; kind: TeamDocument["kind"]; previewUrl?: string; blob?: File }) => {
      const id = uid("doc");
      const name = member?.name ?? "Support";
      setDocuments((prev) => [
        ...prev,
        { id, requestId, name: file.name, kind: file.kind, size: file.size, previewUrl: file.previewUrl, uploadedAt: `Today • ${nowTime()}`, uploadedBy: name },
      ]);
      const messageId = uid("msg");
      setMessages((prev) => [
        ...prev,
        { id: messageId, requestId, author: "team", authorName: name, time: nowTime(), documentId: id, read: false, delivery: "sending" },
      ]);
      touch(requestId, { lastMessage: `Sent a document — ${file.name}` }, `Document sent — ${file.name}`);
      if (liveRef.current) {
        const room = rooms.current[requestId];
        if (!room || !file.blob) {
          setMessages((prev) =>
            prev.map((m) => (m.id === messageId ? { ...m, delivery: "failed", deliveryError: "Upload unavailable." } : m))
          );
          return;
        }
        void (async () => {
          try {
            ...
            if (room.chatRoomId) {
              ...
              setMessages((prev) =>
                prev.map((m) => (m.id === messageId ? { ...m, id: row.id, documentId: doc.id, delivery: "delivered" } : m))
              );
            }
          } catch {
            setMessages((prev) =>
              prev.map((m) => (m.id === messageId ? { ...m, delivery: "failed", deliveryError: "Upload failed." } : m))
            );
          }
        })();
        return;
  ...
  const setStatus = useCallback(
    (requestId: string, status: TeamStatus) => {
      const label = status === "completed" ? "Marked completed" : `Status updated to ${status.replace("-", " ")}`;
      if (liveRef.current) {
        const room = rooms.current[requestId];
        if (room) {
          void requestsApi
            .updateRequestStatus(room.requestId, TEAM_TO_DB_STATUS[status], status === "completed" ? 100 : undefined)
            .catch(() => undefined);
        }
      }
      touch(requestId, status === "completed" ? { status, progress: 100 } : { status }, label);
      setNotifications((prev) => [
        { id: uid("tn"), type: "status", text: `${requestId} status updated to ${status.replace("-", " ")}`, time: nowTime(), read: false, requestId },
        ...prev,
      ]);
    },
    [touch]
    ...
    (requestId: string) => {
      if (liveRef.current) {
        ...
        if (room?.chatRoomId && me) {
          void sendTyping(room.chatRoomId, { userId: me.id, name: me.name, typing: true }).catch(() => undefined);
        }
      }
    },
    []
  ...
  const markRead = useCallback(
    (requestId: string) => {
      ...
      setRequests((prev) => prev.map((r) => (r.id === requestId && r.unread ? { ...r, unread: 0 } : r)));
      if (liveRef.current) {
        const room = rooms.current[requestId];
        if (room?.chatRoomId) void messagesApi.markMessagesSeen(room.chatRoomId, "team").catch(() => undefined);
      }
    },
    [meReader]
  );
    ...
    (messageId: string) => {
      ...
      setMessages((prev) =>
        prev.some((m) => m.id === messageId && m.author === "user" && !m.read)
          ? prev.map((m) => (m.id === messageId ? withReceipt({ ...m, read: true, readAt: at }, meReader, at) : m))
          : prev
      );
    },
    [meReader]
    ...
    (messageId: string, emoji: string) => {
      ...
      setMessages((prev) =>
        prev.map((m) => {
          ...
          const list: TeamReaction[] = m.reactions ? m.reactions.map((r) => ({ ...r, by: [...r.by] })) : [];
  ...
  useEffect(() => {
    ...
    const offs = entries.map(([reference, room]) =>
      subscribeToRoom(room.chatRoomId as string, {
        onMessage: (row) => {
          setMessages((prev) =>
            prev.some((m) => m.id === row.id) ? prev : [...prev, mapTeamMessage(row, reference, member.name)]
          );
        },
        onMessageUpdate: (row) => {
          setMessages((prev) => prev.map((m) => (m.id === row.id ? { ...m, ...mapTeamMessage(row, reference, member.name) } : m)));
        },
        onDocument: (row) => {
          setDocuments((prev) => (prev.some((d) => d.id === row.id) ? prev : [mapTeamDocument(row, reference), ...prev]));
        },
        onTyping: (payload) => {
    ...
    const offRequests = subscribeToRequests((row) => {
      setRequests((prev) => {
        const mapped = mapTeamRequest(row, member.id, prev.find((r) => r.id === (row.reference || row.id))?.userName ?? "User");
        const exists = prev.some((r) => r.id === mapped.id);
        return exists
          ? prev.map((r) => (r.id === mapped.id ? { ...mapped, unread: r.unread, timeline: r.timeline } : r))
          : [{ ...mapped }, ...prev];
      });
    });
    return () => {
  ...
  useEffect(() => {
    ...
    return subscribeToMyNotifications(member.id, (row) => {
      setNotifications((prev) =>
        prev.some((n) => n.id === row.id) ? prev : [mapTeamNotification(row, refByRequestId.current), ...prev]
      );
    });
  }, [live, member]);

  const unreadFor = useCallback((requestId: string) => unreadByRequest[requestId] ?? 0, [unreadByRequest]);
  ...
  const markNotificationRead = useCallback((id: string) => {
    ...
    setNotifications((prev) =>
      prev.some((n) => n.id === id && !n.read) ? prev.map((n) => (n.id === id ? { ...n, read: true } : n)) : prev
    );
  }, []);

  const markNotificationUnread = useCallback((id: string) => {
    setNotifications((prev) =>
      prev.some((n) => n.id === id && n.read) ? prev.map((n) => (n.id === id ? { ...n, read: false } : n)) : prev
    );
  }, []);
