...
export function LatestUpdate({ request, message }: { request: SupportRequest; message: ChatMessage }) {
  return (
    <section>
      <h2 className="text-[11px] font-semibold uppercase tracking-[0.14em] text-text-muted">Latest Update</h2>
      <div className="mt-3 rounded-2xl border border-white/10 bg-surface-1 p-4 sm:p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="text-sm leading-relaxed text-white">{message.text ?? `Shared ${message.file?.name}`}</p>
