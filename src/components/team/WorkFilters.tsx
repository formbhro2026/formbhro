    ...
    <div className="border-b border-border-subtle p-3">
      ...
      <div className="mt-2 flex items-center gap-2 overflow-x-auto pb-1">
        {STATES.map((s) => (
          <button
            ...
            className={cn(
              ...
              values.state === s.key ? "border-brand/40 bg-brand/10 text-brand-light" : "border-border-strong px-2.5 text-text-secondary hover:bg-white/5"
            )}
          >
            {s.label}
          </button>
        ...
        <button
          ...
          className={cn(
            ...
            activeCount > 0 ? "border-brand/40 bg-brand/10 text-brand-light" : "border-border-strong px-2.5 text-text-secondary hover:bg-white/5"
          )}
        >
          <SlidersHorizontal className="h-3 w-3" aria-hidden="true" />
          Filters
          {activeCount > 0 && <span aria-label={`${activeCount} filters active`}>({activeCount})</span>}
        </button>
      </div>

      {open && (
        <div id={`${id}-panel`} className="mt-2 grid grid-cols-2 gap-2 rounded-xl border border-border-subtle bg-surface-2/60 p-2">
          <div>
            <label htmlFor={`${id}-user`} className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-text-muted">
              User
            </label>
            <select id={`${id}-user`} value={values.user} onChange={(e) => onChange({ user: e.target.value })} className={selectClass}>
          ...
          <div>
            <label htmlFor={`${id}-type`} className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-text-muted">
              Request type
            </label>
            <select id={`${id}-type`} value={values.type} onChange={(e) => onChange({ type: e.target.value })} className={selectClass}>
          ...
          <div>
            <label htmlFor={`${id}-rid`} className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-text-muted">
              Request ID
            </label>
            <input
              id={`${id}-rid`}
          ...
          <div>
            <label htmlFor={`${id}-priority`} className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-text-muted">
              Priority
            </label>
            <select
              id={`${id}-priority`}
              value={values.priority}
              onChange={(e) => onChange({ priority: e.target.value as WorkFilterValues["priority"] })}
          ...
          <div className="col-span-2">
            <label htmlFor={`${id}-sort`} className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-text-muted">
              Sort
            </label>
            <select
              id={`${id}-sort`}
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        <p aria-live="polite" className="text-[11px] text-text-muted">
          Showing <span className="font-semibold text-white">{shown}</span> of {total} {total === 1 ? "request" : "requests"}
        </p>
