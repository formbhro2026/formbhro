...
function AdminAnalytics() {
  ...
  const stats = useMemo(() => {
    ...
    const avgCompletion = completed.length
      ? completed.reduce((sum, r) => sum + hoursBetween(r.created_at, r.completed_at!), 0) / completed.length
      : 0;
    const assigned = requests.filter((r) => r.assigned_at);
    const avgResponse = assigned.length
      ? assigned.reduce((sum, r) => sum + hoursBetween(r.created_at, r.assigned_at!), 0) / assigned.length
      : 0;
    ...
    const perTeam = team
      .map((t) => {
        ...
        const lastActivity = mine.length > 0 ? Math.max(...mine.map(r => new Date(r.last_activity_at || r.created_at).getTime())) : 0;
    ...
    return { avgCompletion, avgResponse, daily: since(1), weekly: since(7), monthly: since(30), perTeam, topUsers, topDocs };
  }, [requests, team, documents, profileOf]);

  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
        <StatCard label="Total users" value={roles.filter((r) => r.role === "user").length} icon={Users} />
        ...
        <StatCard label="Avg completion" value={`${stats.avgCompletion.toFixed(1)} h`} icon={Timer} />
        <StatCard label="Avg first response" value={`${stats.avgResponse.toFixed(1)} h`} icon={Clock} />
        <StatCard label="Requests / 24h" value={stats.daily} icon={Inbox} />
        <StatCard label="Requests / 7d" value={stats.weekly} icon={Inbox} hint={`${stats.monthly} in the last 30 days`} />
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <Panel title="Team Engagement & Status">
          <ul className="space-y-2 text-xs">
              ...
              <li key={t.id} className="rounded-xl border border-border-subtle bg-bg px-3 py-2.5">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 truncate">
                    <span className={`h-1.5 w-1.5 rounded-full ${t.isOnline ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-surface-3'}`} />
            ...
            {!stats.perTeam.length && <li className="py-6 text-center text-text-muted">No team members yet.</li>}
          </ul>
        </Panel>
        ...
        <Panel title="Platform Growth (Requests)">
          <div className="space-y-6 py-2">
            <div className="flex items-end justify-between gap-1 h-32 px-2">
              {[stats.daily, stats.weekly / 7, stats.monthly / 30, (stats.monthly * 1.2) / 30].map((val, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-2 group">
                  <div 
                    className="w-full bg-brand/20 group-hover:bg-brand/40 transition-all rounded-t-sm relative" 
                    style={{ height: `${Math.max(10, (val / (Math.max(stats.daily, 1) * 2)) * 100)}%` }}
                  >
        ...
        <Panel title="Most Active Users">
          <ul className="space-y-2 text-xs">
            {stats.topUsers.map(([id, count]) => (
              <li key={id} className="flex items-center justify-between gap-2 rounded-xl border border-border-subtle bg-bg px-3 py-2.5">
                <div className="flex items-center gap-2 truncate">
                   ...
                   <span className="truncate text-white font-medium">{profileOf(id)?.full_name ?? "User"}</span>
                </div>
                <span className="text-[10px] font-bold text-text-muted uppercase">{count} REQS</span>
              </li>
            ))}
            {!stats.topUsers.length && <li className="py-6 text-center text-center text-text-muted">No activity yet.</li>}
          </ul>
        </Panel>
      </div>
    </div>
