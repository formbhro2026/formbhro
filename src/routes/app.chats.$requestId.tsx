...
export const Route = createFileRoute("/app/chats/$requestId")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Chat with the Formbhro support team, share documents and track your request status." },
      { property: "og:title", content: "Conversation — Formbhro Support" },
      { property: "og:description", content: "Chat with support, share documents and follow your request." },
...
function ChatScreen() {
  ...
  const { requests, getRequest, messagesFor, documentsFor, documents, sendMessage, retryMessage, attachFile, addNote, markRead } =
    useUserStore();
      ...
      <>
        ...
        <main className="mx-auto w-full max-w-xl flex-1 px-4 pb-28 pt-8">
          <EmptyState
            ...
            action={
              <Link to="/app/chats" className="rounded-full border border-white/15 px-4 py-2 text-xs font-semibold text-white hover:bg-white/5">
                Go to My Chats
              </Link>
            }
          />
    ...
    >
      <div className="grid min-h-0 flex-1 xl:grid-cols-[minmax(0,20rem)_minmax(0,1fr)] 2xl:grid-cols-[minmax(0,20rem)_minmax(0,1fr)_minmax(0,20rem)]">
        ...
        <section className="flex min-h-0 min-w-0 flex-col">
          ...
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain bg-chat-bg px-3 py-4 pb-4 sm:px-4 relative" style={{ backgroundImage: 'url("https://w0.peakpx.com/wallpaper/580/650/HD-wallpaper-whatsapp-background-cool-dark-grey-google-material-minimal-white.jpg")', backgroundBlendMode: 'soft-light', backgroundSize: '400px', opacity: 1 }}>
            <div className="sticky top-0 z-10 flex justify-center pb-4 pointer-events-none">
              <p className="rounded-lg bg-surface-2/90 backdrop-blur-sm px-3 py-1 text-[11px] font-bold text-text-muted shadow-sm uppercase tracking-wider">TODAY</p>
            </div>
            {messages.map((m) => (
              <MessageBubble key={m.id} message={m} onRetry={retryMessage} onViewFile={setPreviewId} />
            ))}
            <div ref={endRef} />
          </div>
          <div className={cn("transition-[padding] bg-surface-1", keyboardOpen ? "pb-0" : "pb-[76px] lg:pb-0")}>
            <MessageComposer
              requestLabel={request.id}
              onSend={(text) => sendMessage(requestId, text)}
              onUpload={(name, kind, size, preview, file) => attachFile(requestId, name, kind, size, preview, file)}
              onTyping={() => {
