-- Fix the user_roles check in get_admin_analytics which breaks when users have both 'user' and 'admin' roles
...
BEGIN
  ...
  -- FIXED: use role = 'admin' AND LIMIT 1 to prevent multiple row error
  SELECT role INTO v_role FROM public.user_roles WHERE user_id = auth.uid() AND role = 'admin' LIMIT 1;
  ...
  SELECT 
    ...
    COUNT(*) FILTER (WHERE status IN ('assigned', 'waiting_documents', 'under_review', 'in_progress')),
  ...
  INTO v_total, v_pending, v_processing, v_completed, v_avg_completion, v_avg_response, v_daily, v_weekly, v_monthly
