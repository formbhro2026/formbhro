-- Migration: phase7_analytics_and_team
-- Description: Adds permissions and availability to team_members, augments get_admin_analytics with timeseries, and adds missing B-Tree indices.

-- 1. Add permissions and availability to team_members
ALTER TABLE public.team_members 
ADD COLUMN IF NOT EXISTS permissions jsonb DEFAULT '{}'::jsonb NOT NULL,
ADD COLUMN IF NOT EXISTS availability_status text DEFAULT 'online' NOT NULL;

-- 2. Add Missing B-Tree Indices for heavily queried foreign keys
CREATE INDEX IF NOT EXISTS requests_assigned_team_id_idx ON public.requests(assigned_team_id);
...
CREATE INDEX IF NOT EXISTS activity_logs_request_id_idx ON public.activity_logs(request_id);
CREATE INDEX IF NOT EXISTS documents_request_id_idx ON public.documents(request_id);

-- 3. Augment Admin Analytics RPC with Timeseries data
