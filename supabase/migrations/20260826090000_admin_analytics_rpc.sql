-- Migration: admin_analytics_rpc
-- Description: Adds a server-side RPC for Admin Analytics to eliminate unbounded client-side aggregation.
...
BEGIN
  ...
  SELECT json_agg(
    json_build_object(
      ...
      'isOnline', COALESCE(r.last_act, '1970-01-01'::timestamp) > now() - interval '30 minutes'
    )
  ) INTO v_per_team
  ...
  -- 5. Return matched payload for UI
