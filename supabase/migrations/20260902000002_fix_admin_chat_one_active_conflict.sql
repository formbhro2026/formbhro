-- Migration: Ensure get_or_create_admin_team_chat reuses existing active request and handles requests_one_active_per_user safely
...
BEGIN
  ...
  SELECT * INTO v_req
   ...
   WHERE (category = 'Team Direct Report' OR title ILIKE 'Direct Chat%')
     AND (user_id = p_team_member_id OR assigned_team_id = p_team_member_id)
     AND status NOT IN ('completed', 'cancelled')
   ORDER BY created_at DESC
   LIMIT 1;

  -- 2. If not found by category/title, check if user already has ANY active request (to obey requests_one_active_per_user)
  ...
  -- 3. If still no active request found, insert exactly one with ON CONFLICT resolution
  IF v_req.id IS NULL THEN
    ...
    ON CONFLICT (user_id) WHERE status NOT IN ('completed', 'cancelled')
    DO UPDATE SET
      assigned_team_id = EXCLUDED.assigned_team_id,
      assigned_at = COALESCE(requests.assigned_at, now())
  ...
  -- 4. Ensure corresponding chat room exists
  ...
  IF v_room_id IS NULL THEN
    ...
    VALUES (v_req.id)
    ON CONFLICT (request_id) DO UPDATE SET last_message_at = chat_rooms.last_message_at
