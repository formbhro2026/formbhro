-- ==========================================
-- FORMBHRO DATABASE SETUP SCRIPT
-- Run this in your Supabase SQL Editor
-- Project: ogjhvmucklbxcewpkiai
-- ==========================================

-- ============ ENUMS ============
CREATE TYPE IF NOT EXISTS public.app_role AS ENUM ('user','team','admin');
CREATE TYPE IF NOT EXISTS public.request_status AS ENUM ('pending','assigned','waiting_documents','under_review','in_progress','completed','cancelled');
CREATE TYPE IF NOT EXISTS public.request_priority AS ENUM ('low','medium','high');
CREATE TYPE IF NOT EXISTS public.message_sender AS ENUM ('user','team','admin','system');
...
-- ============ REQUESTS ============
CREATE SEQUENCE IF NOT EXISTS public.request_reference_seq START 1000;
...
CREATE UNIQUE INDEX IF NOT EXISTS requests_one_active_per_user
  ON public.requests (user_id)
  WHERE status NOT IN ('completed','cancelled');
CREATE INDEX IF NOT EXISTS requests_user_idx ON public.requests (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS requests_team_idx ON public.requests (assigned_team_id, last_activity_at DESC);
...
CREATE INDEX IF NOT EXISTS messages_room_idx ON public.messages (chat_room_id, created_at);
CREATE INDEX IF NOT EXISTS messages_unseen_idx ON public.messages (request_id) WHERE seen = false;
...
CREATE INDEX IF NOT EXISTS notifications_receiver_idx ON public.notifications (receiver_id, is_read, created_at DESC);
...
CREATE INDEX IF NOT EXISTS activity_logs_request_idx ON public.activity_logs (request_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS status_history_request_idx ON public.request_status_history (request_id, created_at DESC);
...
DROP POLICY IF EXISTS "status history read" ON public.request_status_history;
...
DROP TRIGGER IF EXISTS requests_reference ON public.requests;
...
DROP TRIGGER IF EXISTS requests_created ON public.requests;
...
-- Grant admin access to all tables for service role
